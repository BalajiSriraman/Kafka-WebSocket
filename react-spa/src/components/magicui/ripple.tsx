  ...
  return (
    <div className="absolute left-1/2 top-1/2 h-full w-full overflow-visible">
      {Array.from({ length: NUM_CIRCLES }, (_, i) => (
        <div
          key={i}
          className={`absolute -translate-x-1/2 -translate-y-1/2 animate-ripple rounded-full bg-amber-500`}
          style={
            {
              width: MAIN_CIRCLE_SIZE + i * 55,
              height: MAIN_CIRCLE_SIZE + i * 55,
              opacity: MAIN_CIRCLE_OPACITY - i * 0.003,
